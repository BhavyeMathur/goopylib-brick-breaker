...
class Controller(gp.Rectangle):
    ...
    def update(self):
        ...
        tmp = self.x
        self.x = max(min(mousex, self.maxx), -self.maxx)
...
def setup_window():
    ...
    gp.Image("assets/vignette.png", (0, 0), 900, 600).draw(window).transparency = (0.3, 0.3, 0.8, 0.8)
...
def main():
    ...
    def do_controller_collision():
        if controller.contains(*ball.position):
            ball.collide()
            ball.velx = max(min(ball.velx * 0.85 + controller.velx * 0.2, 12), -12)
            ball.y += 12
